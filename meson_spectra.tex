...
made the guess, or what his name is Ð if it disagrees with experiment\\
...
\ba
\cS_{{\rm meson}}=-\frac{1}{16\pi G_5} \int d^5x \sqrt{-g} e^{-2\Phi}\mathrm{Tr}\left[ \left|DX\right|^2+V_m(\Phi,X^2,\mathcal{G}) \right. \nonumber\\
\left.+\frac{1}{2g_5^2}\left(F_A^2 +F_V^2\right) \right] \, .
\label{eqMesonL}
\ea
...
&+& \left(2a_2 \phi +12 a_4 G^2 + 2a_5 \chi^2 + 2 \tanh(g\phi)\right)H^2 + 4 a_5 G\chi H S.
\ea
...
\ea
We see the advantage of solving the background equations (\ref{C} - \ref{G}) in such a way that $\Delta U$ is a function of the dilaton only, as (\ref{eq:traceV}) does not depend on $\Delta U$, and the equations of motion will not depend upon this numerically-derived term.
...
This numerical analysis will be completed in a later work.
